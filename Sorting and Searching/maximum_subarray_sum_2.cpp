...
int main() {
	...
	long long int pfx[N+1]; // * using 1 based indexing for this one , was the better choice *
	for(int i = 1; i <= N; i++) {
		int a; cin >> a;
	...
	// we can keep a sliding window of size B, then find the lowest pfx[j] using multiset // simple easy good beautiful logic
	for(int i = A; i <= N; ++i) {
